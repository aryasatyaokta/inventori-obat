    ...
    <div className='d-flex' id='wrapper'>
        ...
        <nav class='navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4'>
            <div class='d-flex align-items-center'>
                <h2 class='fs-2 m-0 third-text'>Stok In</h2>
            </div>
        ...
        <table class="table table-borderless bg-blue text-center border border-buttom-0">
            <thead>
                <tr class='secondary-bg text-white'>
                    ...
                    <th class="text-light bg-primary" scope="col">Stok Masuk</th>
                    <th class="text-light bg-primary" scope="col">Tanggal Masuk</th>
                    <th class="text-light bg-primary" scope="col">Aksi</th>
                </tr>
            </thead>
            <tbody>
                <tr class='bg-white' v-for="item in stokInData" :key="item.id">
                    <td>{{ item.namaObat }}</td>
                    <td>{{ item.Stok }}</td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-start">
                    <form @submit.prevent="tambahStok">
                        ...
                        <div class="mb-3">
                            <input v-model="Stok" type="text" class="form-control" placeholder="Enter Stok">
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" @click="tambahStok" class="btn btn-primary">Save changes</button>
                </div>
            </div>
        </div>
    </div>
...
const deleteData = async (itemId) => {
    if (confirm("Apakah kamu yakin ingin menghapus data ini?")) {
        try {
            // Hapus data dari koleksi stokIn berdasarkan ID
            await deleteDoc(doc(db, 'stokIn', itemId));
